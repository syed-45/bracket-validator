...
function isNestedCorrectly(nestedBrackets: string): boolean {
  ...
  if (nestedBrackets[0]==='{' || nestedBrackets[0]==='[' || nestedBrackets[0]==='(') {
    ...
    } else {  // we are nested :0
      if (nestedBrackets.length > matchingIndex+1) {
        workStack.push(nestedBrackets.slice(matchingIndex+1,nestedBrackets.length));
      }
  ...
  while (workStack[0]!==undefined) {
    ...
    if (currentNestOrBracket) {      
      if (currentNestOrBracket[0]==='{' || currentNestOrBracket[0]==='[' || currentNestOrBracket[0]==='(') {        
        matchingIndex = findMatchingBracketIndex(currentNestOrBracket)        
        if (matchingIndex === -1) {
          return false
        } else if (matchingIndex === 1 && currentNestOrBracket.length > 2) {
          workStack.push(currentNestOrBracket.slice(2,currentNestOrBracket.length));
        } else {   // we are nested :0
          if (currentNestOrBracket.length > matchingIndex) {
            workStack.push(currentNestOrBracket.slice(matchingIndex+1,currentNestOrBracket.length))
          }
